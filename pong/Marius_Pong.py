...
class PongGame:
    ...
    #Init
    """def __init__(self, x, y, spielerLeben, spielerScore, spielerAnzeige, yellow, white, black, red,
    schlaegerBreite, schlaegerHoehe, schlaeger_x, schlaeger_y, ball_x, ball_y, ball_rad,
    schlaeger_bew, ball_x_bew, ball_y_bew, spielfeld, back_image, back_left, back_top):
        self.x = x
        self.y = y
        self.spielerLeben = spielerLeben
        self.spielerScore = spielerScore
        self.spielerAnzeige = spielerAnzeige
        self.yellow = yellow
        self.white = white
        self.black = black
        self.red = red
        self.schlaegerBreite = schlaegerBreite
        self.schlaegerHoehe = schlaegerHoehe
        self.schlaeger_x = schlaeger_x
        self.schlaeger_y = schlaeger_y
        self.ball_x = ball_x
        self.ball_y = ball_y
        self.ball_rad = ball_rad
        self.schlaeger_bew = schlaeger_bew
        self.ball_x_bew = ball_x_bew
        self.ball_y_bew = ball_y_bew
        self.spielfeld = spielfeld
        self.back_image = back_image
        self.back_left = back_left
        self.back_top = back_top"""
    ...
    #Reseten des Spiels
    def reset():
        global ball_y_bew, schlaeger_bew, ball_x, ball_y
        ball_x = int(x/2)
        ball_y = int(y/2)
    ...
    def schlaegerAktionen():
        global schlaeger_x, schlaeger_bew
        schlaeger_x += schlaeger_bew

    #Bewegungen des Balls
    def bewBall():
        global ball_x, ball_x_bew, ball_y, ball_y_bew
        ball_x += ball_x_bew
        ball_y += ball_y_bew

    #Aufprall Bewegungen des Balls
    def hitBall():
        global ball_y_bew, ball_x_bew, spielerLeben, spielerScore
    ...
    def gameLogik():
        spielfeld.blit(back_image, (back_left,back_top))
        PongGame.lebensAnzeige()
        PongGame.scoreAnzeige()
        PongGame.schlaegerAktionen()
        PongGame.schlaegerBlock()
        pygame.draw.rect(spielfeld, red, (schlaeger_x, schlaeger_y, schlaegerBreite, schlaegerHoehe),0)
        PongGame.bewBall()
        PongGame.hitBall()
        pygame.draw.circle(spielfeld, yellow, (ball_x, ball_y), ball_rad, 0)
...
if __name__ == '__main__':
    #PongGame(x, y, spielerLeben, spielerScore, spielerAnzeige, yellow, white, black, red,
    #schlaegerBreite, schlaegerHoehe, schlaeger_x, schlaeger_y, ball_x, ball_y, ball_rad,
    #schlaeger_bew, ball_x_bew, ball_y_bew, spielfeld, back_image, back_left, back_top)
    
    PongGame.spielAblauf()
